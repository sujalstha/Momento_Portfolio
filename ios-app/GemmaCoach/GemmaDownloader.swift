...
final class GemmaDownloader {
    ...
    /// Marker file written only after a 100%-complete download (all files + prefill
    /// hardlinks). Its presence is the durable "cached locally" signal.
    static var completionMarkerURL: URL {
        modelDirectory.appendingPathComponent(".download_complete")
    }

    /// True when the model is fully downloaded and cached on disk. Lives in the
    /// app's Documents directory (never purged by iOS, survives every login/logout),
    /// so once this is true the model is NEVER downloaded again. Checked by the UI
    /// to skip the download screen on subsequent launches.
    static var isFullyDownloaded: Bool {
        let fm = FileManager.default
        guard fm.fileExists(atPath: completionMarkerURL.path) else { return false }
        // Sanity-check the two heaviest files are still present, in case the user
        // cleared storage but the marker lingered.
        let chunk1 = modelDirectory.appendingPathComponent("chunk1.mlmodelc/weights/weight.bin")
        let embed  = modelDirectory.appendingPathComponent("embed_tokens_per_layer_q8.bin")
        return fm.fileExists(atPath: chunk1.path) && fm.fileExists(atPath: embed.path)
    }

    /// Mark the model directory as excluded from iCloud/iTunes backup. The model is
    /// large and re-downloadable, so per Apple's storage guidelines it shouldn't be
    /// backed up — but it stays in Documents (not Caches) so the OS never evicts it.
    ...
    func download() async throws -> URL {
        ...
        // Already cached from a previous session/login → nothing to download, ever.
        if Self.isFullyDownloaded {
        ...
        // Write the completion marker LAST — only now is the cache 100% complete,
        // so every future launch/login sees it and skips downloading entirely.
        try? Data().write(to: Self.completionMarkerURL)
    ...
    static let allFiles: [GemmaFile] = {
        ...
        // ── Vision encoder — required for YOLO-perception keyframe review ──
        // The run-session perception pipeline sends camera keyframes to Gemma's
        // vision (EngineModel.streamCoachVision), so the vision encoder IS needed.
        f += mlc("", "vision", weight: 320_000_000)

        // video (vision_video, 338 MB) and audio (295 MB) encoders remain OMITTED
        // — not used by the perception pipeline. Restore their mlc() entries +
        // audio sidecars here if multimodal audio/video is added later.
